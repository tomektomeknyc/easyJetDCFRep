...
from regression_3d_chart import show_regression_surface
import time
...
# Tab 4: NPV Simulation
    ...
    with main_tab6:
        ...
        st.write("Available columns in all_news:", all_news.columns.tolist())

# Try detecting the date column
        possible_date_cols = ["Date", "pubDate", "date", "published", "Published"]

        date_col = next((col for col in possible_date_cols if col in all_news.columns), None)

        if date_col is None:
          st.error("❌ Could not find a valid date column in all_news.")
          st.stop()

# Apply parsing
        all_news["_dt"] = all_news[date_col].apply(parse_date)
...
# 3) Merge headline + link into one “News” cell
  ...
  .news-table th { background-color: #FFA500; color: #000; }
        ...
        # Tab 7: Regression (3D surface of EZJ vs FTSE100 & Jet Fuel)
